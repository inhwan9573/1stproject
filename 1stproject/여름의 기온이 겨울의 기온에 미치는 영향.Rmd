...
우리는 1차 분석을 통해 여름과 겨울 기온만 비교했을 때는 상관관계가 크게 나타나지 않았다고 보았다. 따라서 여름의 다른 기상데이터들이 겨울철 기온과 관련이 있는지를 알아보기 위해 2차 분석을 했다. 아래와 같이 correlation chart를 이용해 overall distribution을 알 수 있다. 
...
앞선 상관관계 분석을 통해 설명변수 간의 상관계수가 다소 높게 나왔다는 것을 알 수 있다. 그에 따른 다중공선성과 고차원적인 문제를 해결할 필요성이 있다. 따라서 주성분분석(PCA)을 통해 차원축소를 하고 쓸모없는 변수를 찾아 제거한다. 
...
'여름이 더우면 겨울이 춥다'는 속설은 기온과의 **상관관계가 거의 없는**것으로 나타나기 때문에 속설일 뿐이라는 결론을 얻을 수 있었다. 기상청에서 조사한 다른 데이터들을 통해 여름의 강수량, 최소습도, 일조시간, 최대풍속이 겨울철 기온과 유의미한 상관 관계가 있음을 알아낼 수 있었다.

하지만 이 변수들이 겨울철 온도와 관계가 있다는 과학적 근거나 자료를 찾을 수 없었다. 따라서 **통계적으로는 유의미하나 겨울 기온과 상관관계가 있다고 말하기는 어렵다**고 볼 수 있다. 
