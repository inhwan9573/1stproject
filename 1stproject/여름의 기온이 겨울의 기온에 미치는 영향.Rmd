...
**여름이 더워질수록 그 해 겨울이 추워진다**는 속설이 있다. 실제로 우리나라의 연간 기온차는 OECD 국가 중 두번째로, 캐나다 다음으로 여름과 겨울의 기온차가 크다는 [자료](https://visualisingkorea.com/ko/2018/08/01/koreas-weather-most-extreme-in-oecd-kor/)가 있다. 
...
## 분석 방법
 ...
 나아가 더운 여름의 어떤 기상 데이터가 겨울의 기온에 영향을 미치는지도 알아볼 수 있다. 이 때 서울시의 여름 기상 데이터를 독립변수로, 겨울 기온데이터를 종속변수로 설정한다. 분석 방법은 **상관관계 분석,회귀분석** 등을 이용한다. 분석을 통해 어떤 변수값이 겨울의 기온에 영향을 가장 많이 미치는 지를 알아낸다.
...
우리나라는 지형이 복잡하다. 그렇기 때문에 분석 범위를 전국으로 설정하면, 지리적 위치에 따라 기온의 편차가 크게 달라질 수 있기 때문에 양질의 데이터를 얻지 못한다. 따라서 분석 범위를 **서울시로 한정**한다. 

## 30개년 데이터 추출

여름은 12개월 중 평균기온이 가장 높았던 3달, 겨울은 12개월 중 평균기온이 가장 낮았던 3달로 정의하여 기상 데이터를 수집한다. 중심극한정리에 따라 데이터의 정규성을 확보하기 위해 1989년 여름부터 2018년 겨울까지 **최근 30개년의 데이터**를 표본으로 준비한다.
...
기상청 기상자료개방포털에서 서울시의 1989년 6월부터 2019년 2월까지의 기상데이터를 내려받는다. 데이터를 정제하기 위해 RStudio에 데이터를 불러온다.
...
앞선 데이터 수집 과정에서 여름은 12개월 중 평균기온이 가장 높았던 3달로 설정하기로 하였다. 데이터를 살펴보면 해마다 6,7,8월의 평균 기온이 가장 높다. 기상청에서 특정 월의 데이터만 따로 내려받을 수 없기 때문에 전체 데이터에서 6,7,8월만 추출하기로 한다. 이 자료에 열지수 데이터는 포함되어있지 않기 때문에 기상청 기상자료개방포털에서 따로 내려받아 먼저 추출한 데이터와 합친다.  
...
### 겨울철 데이터 만들기
여름과 같은 방법으로 겨울 데이터를 추출한다. 12개월 중 평균기온이 가장 낮았던 12월, 이듬해 1,2월을 같은해 겨울로 설정하였다. 여름철 데이터를 만들때와 마찬가지로 전체 데이터에서 12,1,2월 자료를 추출하고 같은 해로 묶어 데이터를 만든다.
```{r}
djf <- data.frame()
...
여름의 기온과 겨울의 기온이 관련이 있는지 알아보기 위해 최근 30개년의 평균기온을 비교한다. 
...
여름과 겨울 평균기온을 그래프로 비교한다. 해가 지날수록 여름의 평균기온은 오르고 겨울의 평균기온은 여름만큼은 아니지만 낮아지는 것으로 확인된다.
<br>
```{r}
#여름평균기온
...
앞선 일평균 기온은 하루의 최고 최저기온의 평균이기 때문에 일교차가 큰 날의 경우는 평균의 오류가 있을 수 있다. 따라서 여름 최고기온과 겨울 최저기온을 살펴본다. 평균기온과 마찬가지로 여름은 기온이 높아지고 있고, 겨울은 기온이 낮아지는 것이 평균기온 그래프보다 더 잘 나타난다.
```{r}
...
기온 자료만 비교하면 연도별로 상대적인 차이만 알 수 있다. 특보자료는 아래 특보 기준을 보면 알 수 있듯이 객관적인 수치이므로 폭염 특보와 한파특보의 개수를 비교하여 추이를 확인한다. 그래프를 보면 대체적으로 폭염특보의 개수가 늘어나면 한파특보의 개수도 늘어나는 것으로 보인다.
...
덥다, 춥다의 개념은 사람이 느끼는 정도이기 때문에 여름은 열지수, 겨울은 체감온도를 이용해 사람이 느끼는 더위와 추위의 기준을 제시한다. 열지수와 체감온도의 정의는 다음과 같다.
...
우리는 1차분석을 통해 여름과 겨울의 기온이 관련이 있다고 보았다. 실제로 여름의 기상데이터들이 겨울철 기온과 관련이 있는지를 알아보기 위해 correlation chart를 이용했다. 이를 통해 overall distribution을 알 수 있다. 
...
앞의 데이터를 더 쉽게 보기 위해 corr plot을 그려 확인했다. 겨울 기온들을 세분화 하여 여름의 기상 데이터들과 유의성이 있는지를 확인한다.
```{r}
library(GGally)
```
...
서로 연관 가능성이 있는 데이터들을 2차원 형태로 보기 위해 주성분분석을 한다.
...
PCA plot에서는 벡터의 길이에 따라 변수에 끼치는 영향을 알 수 있다. 벡터의 길이가 짧으면 변수에 끼치는 영향이 짧고, 반대로 길이가 길면 변수에 큰 영향을 준다. 직각에 위치한 변수들은 서로 영향이 없다고 볼수 있다. 변수가 몰려있을수록 유사성이 높고 반대 방향으로 수평인 변수들은 서로 음의 상관관계가 크다고 해석할 수 있다.
...
'여름이 더우면 겨울이 춥다'는 속설은 기온과의 **상관 관계가 거의 없는**것으로 나타나기 때문에 속설일 뿐이라는 결론을 얻을 수 있었다. 기상청에서 조사한 다른 데이터들을 통해 여름의 강수량, 최소습도, 일조시간, 최대풍속이 겨울철 기온과 유의미한 상관 관계를 가지고 있음을 알아 낼 수 있었다.

하지만 이 변수들이 겨울철 온도와 관계가 있다는 과학적 근거나 자료를 찾을 수 없었다. 따라서 **통계적으로는 유의미하나 겨울의 기온과 상관 관계가 있다고 말하기는 어렵다**고 볼 수 있다. 
